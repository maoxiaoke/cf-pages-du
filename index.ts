...
const getType: typeof mime.getType =
  "getType" in mime
    ? mime.getType
    : // @ts-ignore
      (mime as unknown as { default: typeof mime }).default.getType;
...
async function doRequestWithRetries<T = unknown>(
  url: string,
  apiKey: string,
  options: RequestOptions = {},
  retries = 5
  ...
  while (retryCount++ !== retries) {
    try {
      ...
      (options.log ?? (() => {}))(
        `Failed to request ${url}, delaying for ${delay} ms: ${e}`
      );
...
async function getJwt({
  ...
  projectName,
  apiKey,
}: CloudflarePagesDirectUploaderOptions) {
  const res = await doRequestWithRetries<{ jwt: string }>(
...
async function getMissingHashes(
  jwt: JWTCache,
  hashes: string[],
  options: CloudflarePagesDirectUploaderOptions
) {
  const res = await doRequestWithRetries<string[]>(
...
async function uploadFile(
  ...
  const res = await doRequestWithRetries(
    "/pages/assets/upload",
    options.apiKey,
    {
      headers: {
...
async function createDeployment(
  body: FormData,
  { accountId, projectName, apiKey }: CloudflarePagesDirectUploaderOptions
) {
  const res = await doRequestWithRetries<{ url: string; id: string }>(
...
type DeploymentFileWithHash = DeploymentFile & {
  hash: string;
    ...
    | { contentBase64: string }
    | { contentBase64: undefined; content: () => Promise<Buffer> }
  );
...
export class CloudflarePagesDirectUploader {
    ...
    for (const file of globFiles) {
      ...
      if (relative === "_headers") {
        ...
        if (!options.redirects)
          options.redirects = await readFile(file, "utf8");
        continue;
  ...
  async deployFiles(
      ...
      promises.push(
        (async () => {
          while (hashesToUpload.length > 0) {
            ...
            const contentBase64 =
              file.contentBase64 ?? (await file.content()).toString("base64");
            const contentType =
              file.contentType ??
              getType(file.filename) ??
              "application/octet-stream";

            await uploadFile(
              jwt,
              [{ contentBase64, contentType, hash: file.hash }],
              this.config
            );
    ...
    const manifest = Object.fromEntries(
      filesWithHash.map((f) => ["/" + f.filename, f.hash])
    );
    ...
    return {
      ...deployment,
      hashes: from(filesWithHash).toObject(
        (x) => x.filename,
        (x) => x.hash
      ),
    };
...
function computeHashB64(b64: string, filename: string) {
  ...
  return blake3hash(b64 + extension)
    .toString("hex")
    .slice(0, 32);
}
